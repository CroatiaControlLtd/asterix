...
public:
   ...
   enum
    ...
    virtual void ResetWriteErrors(bool bSeq = false, unsigned int n = 0) { (bSeq ? _nSeqWriteErrors : _nWriteErrors) = n; }
    virtual void ResetErrors(bool bSeq = false) { ResetReadErrors(bSeq); ResetWriteErrors(bSeq); }
    virtual void ResetAllErrors() { ResetErrors(false); ResetErrors(true); }

protected: // to be used by derived class for error counting
    virtual void CountReadError() { _nReadErrors++; _nSeqReadErrors++; }
    virtual void CountWriteError() { _nWriteErrors++; _nSeqWriteErrors++; }
};
