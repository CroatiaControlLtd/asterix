...
bool CConverterEngine::Initialize(const char *inputChannel, const char *outputChannel[], const unsigned int nOutput, const unsigned int chFailover)
{
    ...
    if (!CChannelFactory::Instance()->CreateInputChannel(inputDevice, inputDeviceDescriptor, inputFormat, inputFormatDescriptor))
    {
    ...
    for(unsigned int i=0; i<nOutput; i++)
    {
		...
		if (!CChannelFactory::Instance()->CreateOutputChannel(outputDevice, outputDeviceDescriptor, outputFormat,
			outputFormatDescriptor, i>=chFailover, outputHeartbeat))
		{
