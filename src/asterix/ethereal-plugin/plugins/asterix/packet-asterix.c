    ...
    while (pData) {
        if (pData->tree == 1) {
            ...
            if (pData->type == FT_STRINGZ)
                t_item = proto_tree_add_string(tree[tree_depth], asterix_description_pid[pData->pid], tvb,
                                               pData->bytenr, pData->length, pData->val.str);
            else if (pData->type == FT_UINT32)
                t_item = proto_tree_add_uint(tree[tree_depth], asterix_description_pid[pData->pid], tvb, pData->bytenr,
                                             pData->length, pData->val.ul);
            else if (pData->type == FT_UINT32)
                t_item = proto_tree_add_uint(tree[tree_depth], asterix_description_pid[pData->pid], tvb, pData->bytenr,
                                             pData->length, pData->val.ul);
            else if (pData->type == FT_INT32)
                t_item = proto_tree_add_int(tree[tree_depth], asterix_description_pid[pData->pid], tvb, pData->bytenr,
                                            pData->length, pData->val.sl);
            else if (pData->type == FT_BYTES)
                t_item = proto_tree_add_bytes(tree[tree_depth], asterix_description_pid[pData->pid], tvb, pData->bytenr,
